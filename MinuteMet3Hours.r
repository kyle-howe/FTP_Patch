...
#Profile 6 is Chevron1MinuteSO2.csv
...
FTP_export_profile <- tb_FtpExportMonitor %>%
  filter(ProfileId == 3) %>% #This is the profile for the MoE
...
#
## Note, this skips over T002 which isnt in operation right now
#
FTP_export_profile <- FTP_export_profile %>%
                        filter(STA_StationName != "T002")
...
for (i in 1:length(FTP_station_list)){
  ...
  FTP_parameter_list <- FTP_export_profile %>%
    filter(STA_StationName == FTP_station_list[i]) 
  
  Raw_AQ_data <- MV_station_call(station_serial[[1]],start_time,end_time,"01","F")
  #This is a check from the function to see if data exists. If 1 is returned, station
  #doesn't have data, so skip it
  #if(is.na(Raw_AQ_data)) next
  ...
  Raw_AQ_data <- Raw_AQ_data %>%
    ...
    filter(pollutant %in% FTP_parameter_list$MOT_MonitorName) %>%
    filter(!is.na(value)) %>%
    mutate(value = round2(value,1)) %>%
    mutate(stationID = FTP_station_list[i]) %>%
    mutate(avg_period = "01") %>%
    mutate(region = "GVRD") %>%
    mutate(station_name = FTP_parameter_list$STA_Name[1]) %>%
    mutate(units = "") %>%
...
FINAL_ftp_AQ <- FINAL_ftp_AQ %>%
  mutate(avg_period = as.numeric(avg_period)) %>%
  mutate(date = format(date, format = "%Y/%m/%d %H:%M"))

write.table(FINAL_ftp_AQ, file = "MinuteMet3Hours_patch.csv", sep = ",", col.names = FALSE, row.names = FALSE, quote = FALSE)

ftpUpload("MinuteMet3Hours_patch.csv", "sftp://DMZ\\KHowe:greaSebegone@sftp.metrovancouver.org/MetroVancouver/AQMS/MVAQDATAMIN/NONVERIFIED/MinuteMet3Hours_PATCH.csv")
#ftpUpload("MinuteMet3Hours_patch.csv", "sftp://DMZ\\KHowe:greaSebegone@sftp.metrovancouver.org/MetroVancouver/AQMS/KJH_Test/MinuteMet3Hours_PATCH.csv")
#ftpUpload("MinuteMet3Hours_patch.csv", "sftp://DMZ\\KHowe:greaSebegone@sftp.metrovancouver.org/MetroVancouver/AQMS/MVAQDATAMIN/NONVERIFIED/MinuteMet3Hours_DEV.csv")

#/MetroVancouver/AQMS/MVAQDATAMIN/NONVERIFIED
